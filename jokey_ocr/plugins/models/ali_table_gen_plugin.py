...
class AliTableGenPlugin(Plugin):
    """
    DashScope (OpenAI-compatible) table generator.
    ...
    def __init__(
        self,
        api_key: str,                                 # Use a separate key from OpenAI
        model: str = "qwen-plus",                     # configurable model name
        api_base: str = "https://dashscope.aliyuncs.com/compatible-mode/v1",
        out: Optional[str] = None,
        format: str = "xlsx",                         # xlsx|csv|md
    ...
    def _call_openai(self, messages: List[Dict[str, Any]]) -> str:
        ...
        print(">>> calling Ali, please wait...")
        resp = requests.post(f"{self.api_base}/chat/completions",
                             headers=self._headers(),
                             data=json.dumps(payload))  # (connect, read)
    ...
    def _design_tables(self, image_path: str, upstream: PluginOutput) -> Dict[str, Any]:
        ...
        if not (line.startswith("{") and line.endswith("}")):
            raise ValueError("AliTableGen GPT output is not a single-line JSON.")
        data = json.loads(line)
        if not isinstance(data, dict) or "tables" not in data:
            raise ValueError("AliTableGen JSON must contain a 'tables' array.")
        return data
    ...
    def process(self, image_path: str, upstream: Optional[PluginOutput] = None, **kwargs: Any) -> PluginOutput:
        ...
        # 1) design tables using DashScope
